...
const Game = ({ playerColor, depth, onGameOver }) => {
  ...
  const onSquareClick = ({ square, piece }) => {
    ...
    try {
      ...
      chess.move({ from: moveFrom, to: square });
    } catch {
      const hasMoveOptions = getMoveOptions(square);
      if (hasMoveOptions) {
        setMoveFrom(square);
      ...
      {promotionMove && (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-2 bg-black/80 p-2 rounded-lg">
          {["q", "r", "b", "n"].map((p) => {
            ...
            return (
              <div
                className="cursor-pointer p-1.5 rounded-1.5 bg-[#333]"
                key={piece}
              >
                <img
      ...
      {over !== "" && (
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-[#202020] border-2 border-[#c0c0c0] rounded-xl p-6 text-white flex flex-col gap-4 min-w-70">
            <div className="text-2xl font-semibold text-center">{over}</div>

            <div className="flex gap-3 justify-center">
              <button
                onClick={() => {
                  const userWon = over === "You Win!";
                  onGameOver(userWon); // <--- pass result
                }}
